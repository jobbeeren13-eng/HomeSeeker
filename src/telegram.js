...
async function sendAlert(chatId, listing, score, label, dealScore, dLabel, user = null, botOverride = null) {
  ...
  // Clean address — remove Funda marketing labels
  const JUNK_LABELS = ['blikvanger', 'nieuw', 'verhuurd', 'verkocht', 'onder bod', 'beschikbaar'];
  let address = listing.address || '';
  const addressLower = address.toLowerCase();
  if (JUNK_LABELS.some(j => addressLower === j || addressLower.startsWith(j + ' ') || addressLower.endsWith(' ' + j))) {
    // Extract actual street from URL if address is just a label
    const urlMatch = listing.url?.match(/\/([^/]+)\/\d+\/?$/);
    if (urlMatch) {
      address = urlMatch[1].replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    }
  }
  ...
  if (monthlyCost) lines.push(`Estimated monthly costs: €${monthlyCost.toLocaleString('nl-NL')}/mo`);
  ...
  // Smart boost tips — max 3, specific, no categories
  if (user && score < 85) {
    ...
    if (price > maxHuur && inkomen > 0) {
      const currentRatio = Math.round((price / (inkomen / 3)) * 10) / 10;
      const neededIncome = Math.round(price * 3);
      tips.push({ tip: `Your income covers ${currentRatio}× this rent — landlords typically require 3×. Adding a guarantor or co-applicant can close this gap.`, priority: 3 });
    } else if (!user.heeft_borg || user.heeft_borg === 'nee') {
      tips.push({ tip: `Add a guarantor to strengthen your application — especially useful in competitive markets like ${cityDisplay}.`, priority: 2 });
    }
 
    if (!user.met_partner || user.met_partner === 'nee') {
      tips.push({ tip: `Applying with a partner combines your income and significantly improves approval rates.`, priority: 2 });
    }
 
    if (user.application_readiness === 'niet') {
      tips.push({ tip: `Prepare your documents before applying — income proof and ID are required by most landlords in the Netherlands.`, priority: 3 });
    } else if (user.application_readiness === 'bezig') {
      tips.push({ tip: `Finish preparing your documents — incomplete applications are often rejected immediately.`, priority: 2 });
    }
 
    if (user.contract_type === 'zzp') {
      tips.push({ tip: `As a freelancer, include your last 3 years of tax returns and at least one active client contract.`, priority: 2 });
    } else if (user.contract_type === 'tijdelijk') {
      tips.push({ tip: `Include an employer statement confirming your contract renewal — this reassures landlords about income continuity.`, priority: 1 });
    }
 
    tips.push({ tip: `Send a short personal introduction with your application — it sets you apart from anonymous applicants.`, priority: 1 });
 
    tips.sort((a, b) => b.priority - a.priority);
    const topTips = tips.slice(0, 3);
 
    const potentialBoost = Math.min(100, score + topTips.reduce((s, t) => s + t.priority * 4, 0));
 
    lines.push('');
    lines.push(`*Boost to ${potentialBoost}%*`);
    topTips.forEach(t => lines.push(`· ${t.tip}`));
  }
...
module.exports = { createBot, getBot, sendAlert, processWebhookUpdate, clearLetterState, generateStartPayload };s